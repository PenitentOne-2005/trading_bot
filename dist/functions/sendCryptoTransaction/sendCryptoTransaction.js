...
const allowedKeys = ["trx", "usdt", "usdc", "tusd", "dai"];
const menu = {
    reply_markup: {
        inline_keyboard: [
            [
...
const sendCryptoTransaction = async (chatId) => {
    try {
        ...
        const balanceAmount = balance[toLowerCrypto];
        ...
        const amountWithDecimals = new BigNumber(amount).multipliedBy(10 ** decimals);
        const transferTx = await contract.methods
            .transfer(ESCROW_ADDRESS, amountWithDecimals.toFixed())
            .send();
        return sendMessage(chatId, `✅ Успiшно! Криптовалюта перемещiна в ескроу. Очiкує підтвердження вiд покупця про вiдправку коштiв.\n
      ...
      ! На цьому етапi угоду скасувати неможливо, вона проходить через блокчейн.`, menu);
        ...
        return sendMessage(chatId, `❌ Ошибка при отправке: ${error?.message || error}`);
    }
};
