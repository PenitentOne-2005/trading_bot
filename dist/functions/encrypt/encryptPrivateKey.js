...
export const saveEncryptedPrivateKeyToDB = async (userId, privateKey) => {
...
export const getPrivateKeyFromDB = async (userId) => {
    const res = await pool.query(`SELECT encrypted_private_key, iv FROM user_keys WHERE user_id = $1`, [userId]);
    if (res.rowCount === 0)
        return null;
