...
const createBuyOrder = async (props) => {
    ...
    switch (true) {
        case currentState.step === "waitingForCrypto": {
            ...
            return sendMessage(chatId, `💰 Введи сумму ${text}, которую хочешь купить:`);
        }
        ...
        case currentState.step === "waitingForPrice": {
            ...
            await saveRequest("buy", username, currentState.crypto, currentState.amount, price);
            sendMessage(chatId, `✅ Заявка на покупку ${currentState.amount} ${currentState.crypto} по цене ${price} создана!\nКак только заявка будет обработана, ты получишь уведомление!`);
