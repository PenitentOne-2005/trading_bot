...
const dynamicHandlers = {
    ...
    unpublish_: async (data, { chatId }) => {
        ...
        const { rowCount } = await pool.query(`SELECT 1 FROM ${currentDb} WHERE id = $1`, [orderId]);
        if (!rowCount) {
            return sendMessage(chatId, "❌ Оголошення не знайдено або вже видалене");
        }
        await pool.query(`UPDATE ${currentDb}
       SET status = 'unpublished'
       WHERE id = $1`, [orderId]);
        sendMessage(chatId, `✅ Оголошення успiшно знято з публiкацiї.
Оголошення №${orderId} бiльше не вiдображається у списку активних.

Що далi?`, unPublishMenu);
    },
    delete_: async (data, { chatId }) => {
      ...
      `, [orderId, chatId]);
            if (!rowCount) {
                return sendMessage(chatId, "❌ Оголошення не знайдено або воно вже видалене.");
            }
            await pool.query(`
      UPDATE ${currentDb}
      SET status = 'deleted'
      WHERE id = $1
      `, [orderId]);
            sendMessage(chatId, `🗑️ Оголошення №${orderId} успішно видалено. Всi данi про це оголошення були видаленi. Що далi?`, unPublishMenu);
        }
        catch (err) {
